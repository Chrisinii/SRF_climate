...
export const fetchAPI = async(accessToken) => {
    try {
        const NewsRequest = await fetch("https://api.srgssr.ch/srgssr-contents/v1/articles?items=500&bu=SRF&genre=News&sort=releasedAt&order=DESC&publishedDateFrom=2023-01-01T00%3A00%3A00Z&publishedDateTo=2023-03-31T00%3A00%3A00Z", {
            method: 'GET',
            headers: new Headers({
...
fetchAccessToken().then(token => {
    fetchAPITwo(token).then(response => {
        ...
        klimaArtikel.concat (response.data.articles.edges.filter(article => {
            return JSON.stringify(article).includes("Klima");
        }));

        console.log("*******testen concat");
        console.log(klimaArtikel);
...
export const fetchAPITwo = async(accessToken) => {
    try {
        const NewsRequestTwo = await fetch("https://api.srgssr.ch/srgssr-contents/v1/articles?items=500&bu=SRF&genre=News&sort=releasedAt&order=DESC&publishedDateFrom=2023-04-01T00%3A00%3A00Z&publishedDateTo=2023-06-30T00%3A00%3A00Z", {
            method: 'GET',
            headers: new Headers({
                'Authorization': `Bearer ${accessToken}`,
            }), 
...
export const fetchAPIThree = async(accessToken) => {
    try {
        const NewsRequestThree = await fetch("https://api.srgssr.ch/srgssr-contents/v1/articles?items=500&bu=SRF&genre=News&sort=releasedAt&order=DESC&publishedDateFrom=2023-07-01T00%3A00%3A00Z&publishedDateTo=2023-09-30T00%3A00%3A00Z", {
            method: 'GET',
            headers: new Headers({
                'Authorization': `Bearer ${accessToken}`,
            }), 
...
// API Request 20231001 - 20231231
fetchAccessToken().then(token => {
    fetchAPIFour(token).then(response => {
        // console.log(response.data.articles.edges);
...
export const fetchAPIFour = async(accessToken) => {
    try {
        const NewsRequestFour = await fetch("https://api.srgssr.ch/srgssr-contents/v1/articles?items=500&bu=SRF&genre=News&sort=releasedAt&order=DESC&publishedDateFrom=2023-10-01T00%3A00%3A00Z&publishedDateTo=2023-12-31T00%3A00%3A00Z", {
            method: 'GET',
            headers: new Headers({
                'Authorization': `Bearer ${accessToken}`,
            }), 
        ...
        if(NewsRequestFour.error) {
            return NewsRequestFour;
        }

        console.log(NewsRequestFour)
        return await NewsRequestFour.json();
    } catch(error) {
        console.error('Error:', error);  // Log the error to the console
        return { error: { message: 'Error Fetching Data'}};
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log("Klimaartikel = ", klimaArtikel);
